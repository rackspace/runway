"""Tests for r4y.cfngin.lookups.handlers.hook_data."""
...
class TestHookDataLookup(object):
    """Tests for r4y.cfngin.lookups.handlers.hook_data.HookDataLookup."""
    ...
    def test_default(self, cfngin_context):
        ...
        cfngin_context.set_hook_data('fake_hook',
                                     {'nested': {'result': 'good'}})
        var_top = Variable('test', '${hook_data bad_hook::default=something}',
                           variable_type='cfngin')
    ...
    def test_legacy_valid_hook_data(self, cfngin_context):
        ...
        variable = Variable('test', "${hook_data fake_hook::result}",
                            variable_type='cfngin')
    ...
    def test_legacy_invalid_hook_data(self, cfngin_context):
        ...
        cfngin_context.set_hook_data('fake_hook', {'result': 'good'})
        variable = Variable('test', "${hook_data fake_hook::bad_key}",
                            variable_type='cfngin')
        with pytest.raises(FailedVariableLookup) as err, \
