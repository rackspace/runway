...
def test_assume_role(runway_context):
    ...
    stubber.add_response('assume_role', {'Credentials': credentials,
                                         'AssumedRoleUser': assumed_role_user},
                         {'RoleArn': ROLE_ARN,
                          'RoleSessionName': 'runway',
...
def test_assume_role_no_revert_on_exit(runway_context):
    ...
    stubber.add_response('assume_role', {'Credentials': credentials,
                                         'AssumedRoleUser': assumed_role_user},
                         {'RoleArn': ROLE_ARN,
                          'RoleSessionName': 'runway-test',
                          'DurationSeconds': 900})

    assert runway_context.env.aws_credentials != NEW_CREDENTIALS

    with stubber, AssumeRole(runway_context, role_arn=ROLE_ARN,
                             duration_seconds=900, revert_on_exit=False,
                             session_name='runway-test') as result:
        assert runway_context.env.aws_credentials == NEW_CREDENTIALS
        assert result.role_arn == ROLE_ARN
