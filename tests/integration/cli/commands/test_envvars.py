...
PSH_OUTPUT = ('$env:TEST_VAR_01 = "deployment_1"\n'
...
def test_envvars_no_config(caplog, cd_tmp_path):
    ...
    template = ('Runway config file was not found. Looking for one of '
...
def test_envvars_no_env_vars(caplog, cd_tmp_path, cp_config):
    ...
    result = runner.invoke(cli, ['envvars'])
    assert result.exit_code == 1
    assert 'No env_vars defined in %s' % str(cd_tmp_path / 'runway.yml') in \
        caplog.messages
