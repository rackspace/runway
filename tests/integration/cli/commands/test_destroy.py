...
def test_destroy(mock_runway, cd_tmp_path, cp_config):
    ...
    inst.destroy.assert_called_once_with(
        mock_runway.reverse_deployments.return_value
    )
...
def test_destroy_options_deploy_environment(mock_runway, cd_tmp_path, cp_config):
    ...
    cp_config('min_required', cd_tmp_path)
    runner = CliRunner()
    assert runner.invoke(cli, ['destroy', '-e', 'e-option'],
                         input='y\ny\n').exit_code == 0
...
def test_destroy_options_tag(caplog, cd_tmp_path, cp_config,
                             monkeypatch):
    ...
    runner = CliRunner()
    assert runner.invoke(cli, ['destroy',
                               '--tag', 'app:test-app',
                               '--tag', 'tier:iac'],
                         input='y\n').exit_code == 0
    ...
    assert runner.invoke(cli, ['destroy',
                               '--tag', 'no-match'],
                         input='y\n').exit_code == 1
    assert 'No modules found with the provided tag(s): no-match' \
        in caplog.messages
...
def test_destroy_select_module_child_modules(cd_tmp_path, cp_config,
                                             monkeypatch):
...
def test_destroy_select_module_child_modules_all(cd_tmp_path, cp_config,
                                                 monkeypatch):
