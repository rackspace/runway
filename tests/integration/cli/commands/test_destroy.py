...
def test_destroy_options_module(
    caplog: pytest.LogCaptureFixture,
    cd_tmp_path: Path,
    cp_config: CpConfigTypeDef,
    mocker: MockerFixture,
) -> None:
    """Test destroy option --module."""
    ...
    result = runner.invoke(cli, ["destroy", "--ci", "--module", "sampleapp-01.cfn"])
    assert result.exit_code == 0
    # reverse_deployments is called with the selected deployments
    deployment = mock_runway.reverse_deployments.call_args.args[0][0]
    assert len(deployment.modules) == 1
    assert deployment.modules[0].name == "sampleapp-01.cfn"

    # Multiple module names
    result = runner.invoke(
        cli, ["destroy", "--ci", "--module", "sampleapp-01.cfn", "--module", "sampleapp-02.cfn"]
    )
    assert result.exit_code == 0
    deployment = mock_runway.reverse_deployments.call_args.args[0][0]
    assert len(deployment.modules) == 2
    ...
    result = runner.invoke(cli, ["destroy", "--ci", "--module", "sampleapp-0*.cfn"])
    assert result.exit_code == 0
    deployment = mock_runway.reverse_deployments.call_args.args[0][0]
    # Should match sampleapp-01.cfn, sampleapp-02.cfn, parallel_parent (with children), sampleapp-06.cfn
    assert len(deployment.modules) == 4

    # No match error
    result = runner.invoke(cli, ["destroy", "--ci", "--module", "nonexistent-module"])
    assert result.exit_code == 1
    assert "No modules found matching: nonexistent-module" in caplog.messages


def test_destroy_options_module_child_modules(
    cd_tmp_path: Path,
    cp_config: CpConfigTypeDef,
    mocker: MockerFixture,
) -> None:
    """Test destroy option --module with child modules."""
    mock_runway = mocker.patch(f"{MODULE}.Runway", Mock(spec=Runway))
    cp_config("tagged_modules", cd_tmp_path)
    runner = CliRunner()

    # Match child module by name - includes parent with only matching children
    result = runner.invoke(cli, ["destroy", "--ci", "--module", "sampleapp-03.cfn"])
    assert result.exit_code == 0
    deployment = mock_runway.reverse_deployments.call_args.args[0][0]
    assert len(deployment.modules) == 1
    assert deployment.modules[0].name == "parallel_parent"
    ...
    result = runner.invoke(
        cli, ["destroy", "--ci", "--module", "sampleapp-03.cfn", "--module", "sampleapp-04.cfn"]
    )
    assert result.exit_code == 0
    deployment = mock_runway.reverse_deployments.call_args.args[0][0]
    assert len(deployment.modules) == 1
    ...
    result = runner.invoke(cli, ["destroy", "--ci", "--module", "parallel_parent"])
    assert result.exit_code == 0
    deployment = mock_runway.reverse_deployments.call_args.args[0][0]
    assert len(deployment.modules) == 1
    assert deployment.modules[0].name == "parallel_parent"
...
def test_destroy_options_stack(
    cd_tmp_path: Path,
    cp_config: CpConfigTypeDef,
    mocker: MockerFixture,
) -> None:
    """Test destroy option --stack."""
    mock_runway = mocker.patch(f"{MODULE}.Runway", Mock(spec=Runway))
    cp_config("min_required", cd_tmp_path)
    runner = CliRunner()
