...
def get_config():
    ...
    config['cloud_front_headers'] = as_cloud_front_headers(
        config['http_headers']
    )
...
def get_cookie_headers(client_id,
                       oauth_scopes,
                       tokens,
                       domain_name,
                       cookie_settings,
    ...
    user_data = {
        'UserAttributes': [
            {
                'Name': "sub",
                'Value': decoded_id_token.get('sub')
            },
            {
                'Name': 'email',
                'Value': decoded_id_token.get('email')
            }
        ],
    ...
    cookies = {
        ...
        access_token_key: '%s; %s' % (
            ...
            with_cookie_domain(
                domain_name,
                cookie_settings.get('accessToken')
            )
        ),
        refresh_token_key: '%s; %s' % (
            ...
            with_cookie_domain(
                domain_name,
                cookie_settings.get('refreshToken')
            )
        ),
        last_user_key: '%s; %s' % (
            token_user_name,
    ...
    while attempts:
        ...
        # pylint: disable=broad-except
            ...
            if attempts >= 2:
                ...
                time.sleep(
                    (25 * (pow(2, attempts) + random() * attempts)) / 1000
                )
        finally:
            attempts += 1
