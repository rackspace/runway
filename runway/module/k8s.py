...
class K8s(RunwayModule[K8sOptions]):
    ...
    def kubectl_bin(self) -> str:
        ...
        try:
            ...
            if not which("kubectl"):
                self.logger.error("kubectl not available and a version to install not specified")
    ...
    def skip(self) -> bool:
        ...
        if self.options.kustomize_config.is_file():
            LOGGER.info("processing kustomize overlay: %s", self.options.kustomize_config)
            return False
        LOGGER.info(
            ...
            ", ".join(
                ...
                for i in self.options.gen_overlay_dirs(self.ctx.env.name, self.ctx.env.aws_region)
            ),
        )
        return True
    ...
    def plan(self) -> None:
        ...
        if self.skip:
            return
        self.logger.info("kustomized yaml generated by kubectl:\n\n%s", self.kubectl_kustomize())
