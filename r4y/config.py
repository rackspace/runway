...
class ModuleDefinition(ConfigComponent):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self,  # pylint: disable=too-many-arguments
        ...
        """
        self.name = name
        self._path = Variable(name + '.path', path, 'r4y')
        self._class_path = Variable(name + '.class_path', class_path, 'r4y')
        self.type = type_str
        self._environments = Variable(name + '.environments',
                                      environments or {}, 'r4y')
        self._parameters = Variable(name + '.parameters', parameters or {},
                                    'r4y')
        self._env_vars = Variable(name + '.env_vars', env_vars or {}, 'r4y')
        self._options = Variable(name + '.options', options or {}, 'r4y')
        self.tags = tags or {}
...
class DeploymentDefinition(ConfigComponent):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, deployment):
        ...
        References:
            - :class:`module<r4y.config.ModuleDefinition>`
            - :ref:`command-deploy`
            - :ref:`command-destroy`
            - :ref:`command-plan`
...
class TestDefinition(ConfigComponent):
    ...
    config file and use the ``r4y test`` :ref:`command<command-test>`
    to execute them.
    ...
    def __init__(self,
        ...
        self.name = name
        self.type = test_type
        self._args = Variable(self.name + '.args', args or {}, 'r4y')
        self._required = Variable(self.name + '.required', required, 'r4y')
...
class VariablesDefinition(MutableMap):
    ...
    By default, Runway will look for and load a ``r4y.variables.yml`` or
    ``r4y.variables.yaml`` file that is in the same directory as the
    ...
    default_names = ['r4y.variables.yml', 'r4y.variables.yaml']
...
class Config(ConfigComponent):
    ...
    The Runway config file can have two possible names, ``r4y.yml``
    or ``r4y.yaml``. It must be stored at the root of the directory
    ...
    Example:
        .. code-block:: yaml
            ...
            ---
            # See full syntax at https://github.com/onicagroup/r4y
    ...
    accepted_names = ['r4y.yml', 'r4y.yaml']
    ...
    def __init__(self,
        ...
        Keyword Args:
            deployments (List[Dict[str, Any]]): A list of
                :class:`deployments<r4y.config.DeploymentDefinition>`
                that are processed in the order they are defined.
            tests (Optional[List[Dict[str, Any]]]): A list of
                :class:`tests<r4y.config.TestDefinition>` that are
        ...
        References:
            - :class:`deployment<r4y.config.DeploymentDefinition>`
            - :class:`test<r4y.config.TestDefinition>`
    ...
    def load_from_file(cls, config_path):
        ...
        with open(config_path) as data_file:
            ...
            if config_file:
                LOGGER.warning(
                    'Invalid keys found in r4y file have been ignored: %s',
