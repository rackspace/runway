...
Usage:
  r4y (test|preflight)
  r4y (plan|taxi) [--tag <tag>...]
  r4y (deploy|takeoff) [--tag <tag>...]
  r4y (destroy|dismantle) [--tag <tag>...]
  r4y init
  r4y gen-sample [<samplename>]
  r4y whichenv
  r4y envvars
  r4y run-aws <awscli-args>...
  r4y run-python <filename>
  r4y run-stacker <stacker-args>...
  r4y tfenv (install|run) [<tfenv-args>...]
  r4y kbenv (install|run) [<kbenv-args>...]
  r4y -h | --help
  r4y --version
...
Example:
  r4y deploy --tag app:some-app --tag tier:web  # app:some-app AND tier:web
  r4y destroy --tag app:some-app --tag tier:web  # app:some-app AND tier:web
