...
class StaticSite(RunwayModule):
    ...
    def _create_dependencies_yaml(self, module_dir):
        ...
        pre_destroy = [
            {'path': 'r4y.hooks.cleanup_s3.purge_bucket',
             'required': True,
             'args': {
        ...
        if self.parameters.get('staticsite_auth_at_edge', False):
            pre_build = [{
                'path': 'r4y.hooks.staticsite.auth_at_edge.callback_url_retriever.get',
                'required': True,
            ...
            if self.parameters.get('staticsite_create_user_pool', False):
                pre_destroy.append({
                    'path': 'r4y.hooks.staticsite.auth_at_edge.user_pool_id_retriever.get',
                    'required': True,
                ...
                pre_destroy.append({
                    'path': 'r4y.hooks.staticsite.auth_at_edge.domain_updater.delete',
                    'required': True,
                ...
                {'namespace': '${namespace}',
                 ...
                 'stacks': {
                     "%s-dependencies" % self.name: {
                         'class_path': 'r4y.blueprints.staticsite.dependencies.Dependencies',
    ...
    def _create_staticsite_yaml(self, module_dir):
        ...
        pre_build = [{'path': 'r4y.hooks.staticsite.build_staticsite.build',
                      'required': True,
                      'data_key': 'staticsite',
                      'args': build_staticsite_args}]

        post_build = [{'path': 'r4y.hooks.staticsite.upload_staticsite.sync',
                       'required': True,
                       'args': {
        ...
        pre_destroy = [{'path': 'r4y.hooks.cleanup_s3.purge_bucket',
                        'required': True,
                        'args': {'bucket_rxref_lookup': "%s::BucketName" % self.name}}]

        post_destroy = [{'path': 'r4y.hooks.cleanup_ssm.delete_param',
        ...
        if self.parameters.get('staticsite_auth_at_edge', False):
            ...
            pre_build.append({
                'path': 'r4y.hooks.staticsite.auth_at_edge.domain_updater.update',
                'required': True,
                'data_key': 'aae_domain_updater',
                'args': self._get_domain_updater_variables()
            })
            pre_build.append({
                'path': 'r4y.hooks.staticsite.auth_at_edge.lambda_config.write',
                'required': True,
            ...
            post_build.insert(0, {
                'path': 'r4y.hooks.staticsite.auth_at_edge.client_updater.update',
                'required': True,
        ...
        with open(os.path.join(module_dir, '02-staticsite.yaml'), 'w') as output_stream:  # noqa
            yaml.dump(
                {'namespace': '${namespace}',
                 ...
                 'stacks': {
                     self.name: {
                         'class_path': 'r4y.blueprints.staticsite.%s' % class_path,  # noqa
