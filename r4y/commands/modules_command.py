...
from ..r4y_module_type import RunwayModuleType
...
class ModulesCommand(RunwayCommand):
    ...
    def run(self, deployments=None, command='plan'):
        ...
        if deployments is None:
            deployments = self.r4y_config['deployments']
        context = Context(env_name=get_env(self.env_root,
                          ...
                          command=command)
        context.env_vars['RUNWAYCONFIG'] = self.r4y_config_path
    ...
    def _process_deployments(self, deployments, context):
        ...
        for _, deployment in enumerate(deployments):
            ...
            deployment.resolve(context, self.r4y_vars, pre_process=True)
            LOGGER.info("")
            LOGGER.info("")
    ...
    def _deploy_module(self, module, deployment, context):
        ...
        1. Resolves variables in :class:`r4y.config.DeploymentDefinition`
           and :class:`r4y.config.ModuleDefinition`.
        2. Constructs a ``Dict`` of options to be passed to the ``module_class``.
        3. Determine the class to use to execute the
           :class:`r4y.config.ModuleDefinition`, ``cd`` to the module
           directory, and instanteate the class.
        4. Find and execute the command method of the instanteated class.

        Args:
            module (:class:`r4y.config.ModuleDefinition`): The module
                to be deployed.
            deployment (:class:`r4y.config.DeploymentDefinition`): The
                deployment the module belongs to. Used to get options,
                environments, parameters, and env_vars from the deployment level.
            context: (:class:`r4y.context.Context`): Current context instance.

        """
        deployment.resolve(context, self.r4y_vars)
        module.resolve(context, self.r4y_vars)
        module_opts = {
        ...
        path = Path(module, self.env_root, os.path.join(self.env_root, '.r4y_cache'))
        ...
        with change_dir(path.module_root):
            ...
            r4y_module_type = RunwayModuleType(path.module_root,
            ...
            module_instance = r4y_module_type.module_class(
                context=context,
