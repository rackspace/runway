"""Generate a sample :ref:`Runway module<r4y-module>` directory.
...
|       Name         |  Description                                      |
+====================+===================================================+
| ``cdk-csharp``     | `AWS CDK`_ :ref:`module<r4y-module>` using C#  |
+--------------------+---------------------------------------------------+
| ``cdk-py``         | `AWS CDK`_ :ref:`module<r4y-module>` using     |
|                    | Python                                            |
+--------------------+---------------------------------------------------+
| ``cdk-tsc``        | `AWS CDK`_ :ref:`module<r4y-module>` using     |
|                    | TypeScript                                        |
+--------------------+---------------------------------------------------+
| ``cfn``            | `CloudFormation`_ :ref:`module<r4y-module>`    |
|                    | stack with S3 bucket & DDB table (perfect for     |
|                    | storing Terraform backend state)                  |
+--------------------+---------------------------------------------------+
| ``k8s-cfn-repo``   | `Kubernetes`_                                     |
|                    | :ref:`module<r4y-module>` EKS cluster & sample |
...
|                    | :ref:`module<r4y-module>` using Python         |
+--------------------+---------------------------------------------------+
| ``sls-tsc``        | `Serverless Framework`_                           |
|                    | :ref:`module<r4y-module>` using TypeScript     |
+--------------------+---------------------------------------------------+
| ``stacker``        | `Troposphere`_/`Stacker`_                         |
|                    | :ref:`module<r4y-module>` identical the ``cfn``|
|                    | sample but with the template written in python    |
+--------------------+---------------------------------------------------+
| ``static-angular`` | `StaticSite`_                                     |
|                    | :ref:`module<r4y-module>` of a StaticSite and  |
...
|                    | the React framework                               |
+--------------------+---------------------------------------------------+
| ``tf``             | `Terraform`_ :ref:`module<r4y-module>`         |
+--------------------+---------------------------------------------------+
...
.. code-block:: shell
    ...
    # create a "r4y-sample-tfstate.cfn" sample module directory
    $ r4y gen-sample stacker

    # create a "sampleapp.sls" sample module directory
    $ r4y gen-sample sls-py
...
def generate_tfstate_cfn_template():
    ...
    from r4y.blueprints.tf_state import TfState
...
def generate_sample_k8s_cfn_repo(env_root):
    ...
    from r4y.blueprints.k8s.k8s_master import Cluster
    from r4y.blueprints.k8s.k8s_iam import Iam
    from r4y.blueprints.k8s.k8s_workers import NodeGroup as WorkerNodeGroup
...
def generate_sample_cdk_tsc_module(env_root, module_dir=None):
    ...
    for i in ['.npmignore', 'cdk.json', 'package.json', 'r4y.module.yml',
...
def generate_sample_cdk_py_module(env_root, module_dir=None):
    ...
    for i in ['hello/__init__.py', 'hello/hello_construct.py',
              ...
              'package.json', 'Pipfile', 'Pipfile.lock', 'r4y.module.yml']:
        shutil.copyfile(
            os.path.join(ROOT,
                         'templates',
                         'cdk-py',
