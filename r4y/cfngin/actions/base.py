...
def build_walker(concurrency):
    ...
    Passed to :class:`r4y.cfngin.plan.Plan` for walking the graph.
    ...
    Returns:
        Callable[..., Any]: Function to walk a :class:`r4y.cfngin.dag.DAG`.
...
def stack_template_url(bucket_name, blueprint, endpoint):
    ...
    Args:
        ...
        blueprint (:class:`r4y.cfngin.blueprints.base.Blueprint`): The
...
class BaseAction(object):
    ...
    Each action is tied to a :class:`r4y.cfngin.commands.stacker.base.BaseCommand`,
    and is responsible for building the :class:`r4y.cfngin.plan.Plan` that
    ...
    Attributes:
        ...
        context (:class:`r4y.cfngin.context.Context`): The context
            for the current run.
    ...
    def provider(self):
        ...
        Returns:
            :class:`r4y.cfngin.providers.base.BaseProvider`

        """
        return self.provider_builder.build()

    def build_provider(self, stack):
        """Build a :class:`r4y.cfngin.providers.base.BaseProvider`.

        Args:
            stack (:class:`r4y.cfngin.stack.Stack`): Stack the action will
                be executed on.

        Returns:
            :class:`r4y.cfngin.providers.base.BaseProvider`: Suitable for
            operating on the given :class:`r4y.cfngin.stack.Stack`.
