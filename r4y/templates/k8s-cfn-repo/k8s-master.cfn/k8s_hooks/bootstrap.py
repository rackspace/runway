...
def create_r4y_environments(provider, context, **kwargs):  # noqa pylint: disable=unused-argument
    ...
    """
    LOGGER.info("Bootstrapping r4y k8s modules, looking for unconfigured "
                "environments...")
    ...
    with open(os.environ.get('RUNWAYCONFIG')) as data_file:
        r4y_config = yaml.safe_load(data_file)

    for deployment in r4y_config.get('deployments', []):
