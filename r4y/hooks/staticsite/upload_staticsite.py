...
# TODO move to r4y.cfngin.hooks on next major release
import logging
import time
...
from ...commands.r4y.run_aws import aws_cli
...
def display_static_website_url(website_url_handle, provider, context):
    ...
    Keyword Args:
        ...
        provider (:class:`r4y.cfngin.providers.base.BaseProvider`):
            The provider instance.
        context (:class:`r4y.cfngin.context.Context`): context instance
...
def update_ssm_hash(context, session):
    ...
    Keyword Args:
        context (:class:`r4y.cfngin.context.Context`): context instance
        session (:class:`r4y.cfngin.session.Session`): CFNgin session
...
def prune_archives(context, session):
    ...
    Keyword Args:
        context (:class:`r4y.cfngin.context.Context`): The context
            instance.
        session (:class:`r4y.cfngin.session.Session`): The CFNgin
            session.
