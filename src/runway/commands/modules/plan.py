...
:ref:`runway config file<runway-config-options>`. If the
``DEPLOY_ENVIRONMENT`` environment variable is set, it's
value will be used. The environment identified here is used to determine
the env/config files to use. It is also used with options defined in the
runway config file such as ``assume-role``, ``account-id``, etc. See
:ref:`Runway Config<runway-config-options>` for details on these options.
