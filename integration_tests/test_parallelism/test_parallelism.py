...
from integration_tests.utils import copy_dir, copy_file, execute_tests, import_tests
...
class Parallelism(IntegrationTest):
    ...
    def run(self):
        ...
        tests = [
            test(self.logger, self.environment) for test in Parallelism.__subclasses__()
        ]
        if not tests:
    ...
    def clean(self):
        ...
        for file_type in file_types:
            templates.extend(
                glob.glob(os.path.join(self.parallelism_test_dir, file_type))
            )
